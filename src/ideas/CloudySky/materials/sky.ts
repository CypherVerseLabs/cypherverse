...
export const useSkyMat = (radius: number, colors: number[]): ShaderMaterial => {
    ...
    () =>
      new ShaderMaterial({
        uniforms: {
          ...
          num_colors: new Uniform(colors.length / 3),
          time: new Uniform(0),
        },
        vertexShader: vert,
        fragmentShader: frag,
  ...
  useFrame(({ clock }) => {
    ...
    (mat.uniforms.time as Uniform).value = (clock.getElapsedTime() % 10000) / 5;
...
const frag = `
  ...
  float get_mix(int index, float noise) { 
    highp float lastPerc = float(index*3 - 1) / float(num_colors - 1);
    highp float thisPerc = float(index*3) / float(num_colors - 1);
